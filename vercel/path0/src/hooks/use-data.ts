...
export function useData() {
  ...
  const updateCategory = useCallback((type: 'income' | 'expense', oldName: string, newName: string) => {
    ...
    setData(currentData => {
      ...
      const categoryIndex = newCategories[type].indexOf(oldName);
      if (categoryIndex === -1) return currentData;

      newCategories[type][categoryIndex] = newName.trim();
  ...
  const deleteCategory = useCallback((type: 'income' | 'expense', categoryToDelete: string) => {
    setData(currentData => {
        const newCategories = { ...currentData.categories };
        newCategories[type] = newCategories[type].filter(c => c !== categoryToDelete);
  ...
  return { data, loading, addTransaction, updateTransaction, deleteTransaction, addCategory, updateCategory, deleteCategory, exportData, resetData, getTranslatedCategory };
}
